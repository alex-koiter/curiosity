...
# Find station(s)
stations_search(coords = c(49.899065, -100.897145), interval = "day")

# Download data from ECCC
## Two virden stations note data gap 2005 - 2008
data <- weather_dl(station_ids = "47367", start = "2008-10-01", interval = "day") %>%
  bind_rows(weather_dl(station_ids = "3560", start = "1959-10-01", interval = "day"))
...
# 2014 looks like lots of snow
filter(water_year, cum_precip > 2000)

ggplot(data = filter(water_year, water_year == 2014), aes(y = total_precip , x = date)) +
  geom_point()
...
ggsave(plot = p1, filename = "Virden water year precip.png", width = 230, height = 200, units = "mm", dpi = 600)
...
ggsave(plot = p2, filename = "Virden growing precip.png", width = 230, height = 200, units = "mm", dpi = 600)
...
ggsave(plot = p3, filename = "Virden GDD.png", width = 230, height = 200, units = "mm", dpi = 600)

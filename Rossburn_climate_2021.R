...
data <- weather_dl(station_ids = "3458", start = "1959-01-01", interval = "day")
min(data$date)
# There is no temperature data for this station
...
p1 <- ggplot(data = avg, aes(y = median , x = new_date)) +
  ...
  geom_line(data = filter(water_year, water_year == 2021), aes(y = cum_precip , x = new_date), colour = "red", size = 1.5) +
  scale_x_date(date_labels = "%B") +
  labs(y = "Cumulative Precipitation (mm)", x = "Date", title = "Water Year (Oct-Sept) Cumulative Precipitation \nRossburn MB 1986-2020") +
  theme_bw(base_size = 20) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 

p1

ggsave(plot = p1, filename = "Rossburn water year precip.png", width = 230, height = 200, units = "mm", dpi = 600)
...
# Current growing year precipitation
...
ggsave(plot = p2, filename = "Rossburn growing precip.png", width = 230, height = 200, units = "mm", dpi = 600)
